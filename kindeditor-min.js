...
(function(A, o) {
	...
	function S(a, b, c, d, e) {
		var c = c || "",
			...
			a = a.replace(/<(?:br|br\s[^>]*)\s*\/?>\s*<\/br>/ig, "</br>"),
			a = a.replace(/(<(?:br|br\s[^>]*)>)\s*(<\/br>)/ig,
				"$1<br />$2"),
			a = a.replace(/\u200B/g, ""),
			a = a.replace(/\u00A9/g, "&copy;"),
			h = {};
	...
	function oa(a, b) {
		if (a.hasVal()) {
			if (b === o) {
				var c = a.val();
				return c = c.replace(/(<(?:br|br\s[^>]*)>) *(<\/br>)/ig, "")
			}
			return a.val(b)
		}
		return a.html(b)
		...
		if (!(l && d !== "br") && (!ea || !(z < 3 && d !== "p")) && !(La && z < 9)) {
			...
			f(c).keydown(function(f) {
				if (!(f.which !=
					...
					b.cmd.selection();
					var i = a(b.cmd.range);
					i == "marquee" || i == "select" || (d === "br" && !e[i] ? (f.preventDefault(), b.insertHtml("<br />" + (l && z < 9 ? "" : "\u200b"))) : g[i] || P(c, "formatblock", "<br>"))
				}
			});
			f(c).keyup(function(e) {
				if (!(e.which != 13 || e.shiftKey || e.ctrlKey || e.altKey) && d != "br")
					...
					if (g[e] || P(c, "formatblock", "<br>"), e = b.cmd.commonAncestor("div")) {
						for (var i = f("<br></br>"), k = e[0].firstChild; k;) {
							var n = k.nextSibling;
							i.append(k);
							k = n
						}
	...
	if (!A.KindEditor) {
		...
		f.options = {
			...
			minChangeSize: 50,
			zIndex: 811213,
			items: ["source", "|", "preview", "code", "plainpaste", "wordpaste","bold", "|", "justifyleft", "justifycenter", "justifyright", "insertorderedlist", "insertunorderedlist", "indent", "outdent", "subscript", "superscript", "clearhtml", "quickformat", "selectall", "|", "image", "multiimage", "flash", "media", "insertfile", "table", "hr", "emoticons", "baidumap", "pagebreak", "link", "unlink", "|"
			],
		...
		Eb("core", function(a) {
			...
			b.afterCreate(function() {
				function c() {
					...
					var d = j[0].innerHTML;
					j.remove();
					d !== "" && (W && (d = d.replace(/(<br>)\1/ig, "$1")), b.pasteType === 2 && (d = d.replace(/(<(?:br|br\s[^>]*)>) *(<\/br>)/ig, ""), /schemas-microsoft-com|worddocument|mso-\w+/i.test(d) ? d = lb(d, b.filterMode ? b.htmlTags :
						a.options.htmlTags) : (d = S(d, b.filterMode ? b.htmlTags : null), d = b.beforeSetHtml(d))), b.pasteType === 1 && (d = d.replace(/&nbsp;/ig, " "), d = d.replace(/\n\s*\n/g, "\n"), d = d.replace(/<br[^>]*>/ig, "\n"), d = d.replace(/<\/br><br[^>]*>/ig, "\n"), d = d.replace(/<[^>]+>/g, ""), d = d.replace(/ {2}/g, " &nbsp;"), b.newlineTag == "br" ? /\n/.test(d) && (d = d.replace(/^/, "<br>").replace(/$/, "<br /></br>").replace(/\n/g, "<br /></br><br>")) : d = d.replace(/\n/g, "<br />$&")), b.insertHtml(d, !0))
				}
				var d = b.edit.doc,
