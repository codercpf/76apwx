...
K.options = {
	...
	zIndex : 811213,
	items:
	["source", "|", "preview", "code", "plainpaste", "wordpaste", "bold","|", "justifyleft", "justifycenter", "justifyright", "insertorderedlist", "insertunorderedlist", "indent", "outdent", "subscript", "superscript", "clearhtml", "quickformat", "selectall", "|", "image", "multiimage", "flash", "media", "insertfile", "table", "hr", "emoticons", "baidumap", "pagebreak", "link", "unlink", "|"
	],
...
function _formatHtml(html, htmlTags, urlType, wellFormatted, indentChar) {
	...
	html = html.replace(/<(?:br|br\s[^>]*)\s*\/?>\s*<\/br>/ig, '</br>');
	html = html.replace(/(<(?:br|br\s[^>]*)>)\s*(<\/br>)/ig, '$1<br />$2');
	html = html.replace(/\u200B/g, '');
...
function _elementVal(knode, val) {
	if (knode.hasVal()) {
		if (val === undefined) {
			var html = knode.val();
			html = html.replace(/(<(?:br|br\s[^>]*)>) *(<\/br>)/ig, '');
			return html;
		}
		...
		function movePastedData() {
			...
			if (self.pasteType === 1) {
				...
				if (self.newlineTag == 'p') {
					if (/\n/.test(html)) {
						html = html.replace(/^/, '<br>').replace(/$/, '<br /></br>').replace(/\n/g, '<br /></br><br>');
					}
				} else {
					html = html.replace(/\n/g, '<br />$&');
				}
